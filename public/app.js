(() => {
  ...
  async function captureOptInLocation(formElement) {
    ...
    if (!allow?.checked || !navigator.geolocation) return '';
    return new Promise(resolve => navigator.geolocation.getCurrentPosition(
      position => {
        const lat = Number(position.coords.latitude).toFixed(5);
        const lng = Number(position.coords.longitude).toFixed(5);
        resolve(`${lat}, ${lng} | https://www.google.com/maps?q=${lat},${lng}`);
      },
      () => resolve(''),
  ...
  adminChats = async function(main) {
    const users=state.admin.role==='admin'?(await request('/api/admin/users')).users:[];
    main.innerHTML=`<div class="admin-toolbar admin-toolbar-pro"><div><span class="admin-kicker">Hộp thư</span><h1 class="admin-title">Chat trực tuyến</h1><p class="admin-sub">Cuộn riêng vùng tin nhắn như Messenger. Chat đã xong chỉ xem lại.</p></div><button id="chatSound" class="small-btn">Bật chuông chat</button></div><div class="admin-filter-row form-three"><select id="chatStatus"><option value="">Tất cả</option><option value="open">Đang mở</option><option value="done">Đã xong</option></select>${state.admin.role==='admin'?`<select id="chatAssigned"><option value="">Tất cả nhân viên</option>${users.filter(u=>u.role==='employee').map(u=>`<option value="${u.id}">${escapeHTML(u.full_name)}</option>`).join('')}</select>`:''}<input id="chatFrom" type="date"><input id="chatTo" type="date"></div><div class="admin-chat"><div id="conversationList" class="chat-list-admin"></div><div id="conversationThread" class="chat-admin-thread"><div class="admin-empty">Chọn hội thoại để xem.</div></div></div>`;
