(() => {
  ...
  async function openProduct(slug) {
    try {
      ...
      $('#detailLead', modal).onsubmit = async event => {
        event.preventDefault(); const form = new FormData(event.target);
        try { await request('/api/leads', { method:'POST', body:{ type:'product_consultation', product_id:Number(form.get('product_id')), name:form.get('name'), phone:form.get('phone'), note:form.get('note') } }); trackEvent('Lead', { content_name: product.name, content_category: product.category }); notify('Tâm An đã nhận thông tin và sẽ liên hệ sớm.'); event.target.reset(); }
  ...
  function openLeadModal(info = {}) {
    ...
    const close = () => { modal.remove(); document.body.classList.remove('modal-open'); };
    $('.modal-close', modal).onclick = close;
    $('#quickLead', modal).onsubmit = async event => {
      event.preventDefault(); const form = new FormData(event.target);
      try { await request('/api/leads', { method:'POST', body:{ type:info.name || 'consultation', product_id:Number(info.id) || null, name:form.get('name'), phone:form.get('phone'), note:form.get('note') } }); trackEvent('Lead', { content_name: info.name || 'consultation' }); notify('Đã gửi yêu cầu thành công.'); close(); }
      catch (error) { notify(error.message); }
    };
  }
